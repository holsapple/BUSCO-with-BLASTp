...
Modified by R.W. Holsapple (Fall 2016) to try an incorporate the use of BLASTp.
...
class Analysis(object):
    ...
    def __init__(self, params):
        ...
        # rwh_mod_11_01_2016: Initialize the blastp parameter.
        self._blastp = params['blastp']
        # -----
    ...
    def _blast(self, missing_and_frag_only=False, ancestral_variants=False):
        ...
        if not missing_and_frag_only:
            ...
            # rwh_mod_11_01_2016: If '-bp' or '--blastp' option is given, the blast database should be a protein database.
            if self._blastp:
                Analysis.p_open(['makeblastdb', '-in', self._sequences, '-dbtype', 'prot', '-out',
        ...
        # rwh_mod_11_01_2016: If '-bp' or '--blastp' option is given, modify the log info appropriately.
        if self._blastp:
            _logger.info('Running blastp, writing output to %sblast_output/blastp_%s%s.tsv...'
        ...
        # rwh_mod_11_01_2016: If '-bp' or '--blastp' option is given, call blastp on the command line, not tblastn.
        if self._blastp:
            Analysis.p_open(['blastp', '-evalue', str(self._ev_cutoff), '-num_threads', str(self._cpus),
                             '-query', query_file, '-db', '%s%s%s' % (self._tmp, self._abrev, self._random),
                             '-out', '%sblast_output/blastp_%s%s.tsv' % (self.mainout, self._abrev, output_suffix),
                             '-outfmt', '7'], 'blastp', shell=False)
        # -----

        else:
            Analysis.p_open(['tblastn', '-evalue', str(self._ev_cutoff), '-num_threads', str(self._cpus),
                             '-query', query_file, '-db', '%s%s%s' % (self._tmp, self._abrev, self._random),
                             '-out', '%sblast_output/tblastn_%s%s.tsv' % (self.mainout, self._abrev, output_suffix),
                             '-outfmt', '7'], 'tblastn', shell=False)

        # check that blast worked
        # rwh_mod_11_01_2016: If '-bp' or '--blastp' option is given, check for correct filename to check if blast worked.
        if self._blastp:
            if not os.path.exists('%sblast_output/blastp_%s%s.tsv' % (self.mainout, self._abrev, output_suffix)):
                _logger.error('blastp failed !')
                raise SystemExit
        # -----
        ...
        # rwh_mod_11_01_2016: If '-bp' or '__blastp' option is given, check for "processed" in the correct file.
        if self._blastp:
            try:
                if "processed" not in open('%sblast_output/blastp_%s%s.tsv' % (self.mainout, self._abrev, output_suffix), 'r').readlines()[-1]:
                    _logger.warning('blastp might have ended prematurely (the result file lacks the expected final line), '
                                    'which could produce incomplete results in the next steps !')
            except IndexError:
                pass  # if the blastp result file is empty, for example in phase 2 if 100% was found in phase 1
        # -----

        else:
            try:
                if "processed" not in open('%sblast_output/tblastn_%s%s.tsv' % (self.mainout, self._abrev, output_suffix), 'r').readlines()[-1]:
...
class GenomeAnalysis(Analysis):
    ...
    def run_analysis(self):
        ...
        # rwh_mod_11_01_2016: If '-bp' or '--blastp' option is given, run _check_protein(), otherwise run _check_nucleotide()
        if self._blastp:
            self._check_protein()
        else:
    ...
    def _get_coordinates(self, missing_and_frag_only=False):
        """
        This function gets coordinates for candidate regions from blast result file
        :param missing_and_frag_only: tell whether to use the missing_and_frag_rerun tblastn file
        :type missing_and_frag_only: bool
        """

        # rwh_mod_11_02_2016: If '-bp' or '--blastp' option is given, open the appropriate blast .tsv file.
        if self._blastp:
            if missing_and_frag_only:
                blast_file = open('%sblast_output/blastp_%s_missing_and_frag_rerun.tsv' % (self.mainout, self._abrev))
            else:
                blast_file = open('%sblast_output/blastp_%s.tsv' % (self.mainout, self._abrev))
        # -----
...
class TranscriptomeAnalysis(Analysis):
    ...
    def run_analysis(self):
        ...
        # rwh_mod_11_17_2016: If '-bp' or '--blastp' option is given, run _check_protein(), otherwise run _check_nucleotide()
        if self._blastp:
            self._check_protein()
        else:
            self._check_nucleotide()
    ...
    def _get_coordinates(self):
        ...
        # rwh_mod_11_18_2016: If '-bp' or '--blastp' option is given, open blastp.tsv, otherwise open tblastn.tsv
        if self._blastp:
            f = open('%sblast_output/blastp_%s.tsv' % (self.mainout, self._abrev))  # open input file
        else:
...
def _parse_args():
    ...
    # rwh_mod_11_01_2016: added blastp optional argument.
    optional.add_argument(
        '-bp', '--blastp', action='store_true', required=False, default=False, dest='blastp',
        help='Blast using BLASTp instead of tBLASTn.')
    # -----
...
def _define_parameters(args):
    ...
    return {"mode": mode, "target_species": target_species, "abrev": args['abrev'], "tmp": args['tmp'],
            ...
            "dataset_nb_buscos": dataset_nb_buscos, "blastp": args['blastp']  # rwh_mod_11_01_2016: added blastp argument.
            }
